...
/**
    ...
    n if these g-steps prime numbers exist otherwise nil or null or None or Nothing or [] or "0, 0" or {0, 0} or 0 0 or
    ...
    ([193, 197] is also such a 4-steps primes between 130 and 200 but it's not the first pair).
        ...
        step(6, 100, 110) --> [101, 107] though there is a prime between 101 and 107 which is 103; the pair 101-103 is
        a 2-step.
...
fun step(g:Int, m:Long, n:Long): LongArray {
    val result : LongArray = longArrayOf(0, 0)

    if ((g < 2) || (m < 2) || (n < 2) || (m >= n) || ((n-m)<g)) return longArrayOf()

    for (number in m..n){
        if (isPrimeNumber(number) && (isPrimeNumber(number+g))){
            result[0] = number
            result[1] = number+g
            return result
        }
...
fun isPrimeNumber(isThisNumberPrime : Long) : Boolean {
    for (number in 2 .. sqrt(isThisNumberPrime.toDouble()).toLong()){
        if(isThisNumberPrime % number == 0L){
            return false
        }
    }
    return true
