...
/*
    You have a positive number n consisting of digits. You can do at most one operation: Choosing
    the index of a digit in the number, remove this digit at that index and insert it back to
    another or at the same place in the number in order to find the smallest number you can get.
...
fun smallest(n: Long): LongArray {
    ...
    for ( numberToMove in 0 until number.length){

        for ( positionToMoveNumber in 0 until number.length){
            val switchedNumberArrayList = ArrayList(numberAsArrayList)

            switchedNumberArrayList.removeAt(numberToMove)
            switchedNumberArrayList.add(positionToMoveNumber, number[numberToMove].toString())

            val switchedNumber = switchedNumberArrayList.joinToString("").toLong()

            if (switchedNumber < minNumber){
                minNumber = switchedNumber
                indexI = numberToMove.toLong()
                indexJ = positionToMoveNumber.toLong()
            }
        }
    }
